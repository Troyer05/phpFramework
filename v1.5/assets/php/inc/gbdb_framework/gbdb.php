...
class GetForm {
     ...
     * @return bool true wenn es keine Probleme gab
     */
    public static function upload(mixed $file, string $path): bool {
 ...
 * - Stellen Sie sicher, dass Sie die neuesre Version des GBDB Backends auf dem SecondServer haben und dieses über die von Ihnen angegebene Adresse erreichbar ist.
...
class SecondServer {
    /**
     * Verarbeitet die SecondServer Antwort
     * @internal Used by Framework
     */
    private static function proc(mixed $data): bool {
        $con = false;
        ...
        if (!$con) {
            if (Vars::enable_second_server_warnings()) {
                echo "Warnung: Bei dieser Anfrage ist etwas auf dem SecondServer schief gelaufen. Überprüfen Sie die Anfrage.<br><br>";
            }
        }
    ...
    /**
     * Fordert einen Sicherheits Token an für die anstehende Datenübertragung
     * @internal Used by Framework
     */
    private static function secure(): mixed {
        $s = Vars::second_server_security();
        $tmp = Api::fetch(Vars::second_server_address(), [], ["request" => "authentification", "use" => $s]);
        $auth = $tmp[0]["auth"];
        $verify = $tmp[0]["verify"];

        return ["auth" => $auth, "verify" => $verify];
    }

    /**
     * Test die Verbindung zum SecondServer
     * @return bool true, wenn der SecondServer korrekt antwortet
     */
    public static function testConnection(): bool {
        $connection = false;
        $tmp = Api::fetch(Vars::second_server_address(), [], ["request" => "test"]);

        if (isset($tmp[0]["response"])) {
            if ($tmp[0]["response"] == "ok") {
                $connection = true;
            }
        }
        ...
        if (!$connection) {
            if (Vars::enable_second_server_warnings()) {
                echo "Warnung: Der SecondServer gibt keine korrekte Antwort und ist daher womöglich nicht erreichbar bzw. Einsatzbereit.<br><br>";
            }
        }
    ...
    /**
     * Sendet Daten an den SecondServer (@example Vorgefertige Anfragen sind in test.php zu finden)
     * @param mixed $request Anfrage an den SecondServer welche die Daten beinhaltet
     * @return bool true, wenn alles funktioniert hat
     */
    public static function sendData(mixed $request): bool {
        $secure = self::secure();

        $body = [
            "security" => $secure,
            "request" => "send_data",
            "special_action" => "store_data",
            "use_system" => "gbdb_backend",
            "request_data" => $request
        ];

        $res = Api::fetch(Vars::second_server_address(), [], $body);
